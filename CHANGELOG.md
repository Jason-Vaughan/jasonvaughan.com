...
- **Added and refined Kobold agent card** — integrated the new `Kobold` edge node into the `OpenClaw Fleet` list in `src/components/OpenClawFleet.jsx` and the RAG chat database in `public/chat-context.json`. Configured its role as a voice-operated, hands-free `AI Assistant Video Engineer` that connects directly to local show networks on-site to scan, monitor, and query local video devices and resources, utilizing Monad-1 over Tailscale as its local LLM backend. Incorporated the official Toxic Green accent theme (`#8BC34A`), copy from the Kobold branding kit, and circular reptilian eye avatar. Set the card as a redacted stealth node (`redacted: true`, `REDACTED · Stealth` badge) with no banner to keep the card layout clean. Rendered its widescreen advertisement graphic as a clickable thumbnail aligned on the right side of the card (wrapping on mobile), opening a premium blurred zoom modal with an "Apply for Guild Membership" CTA linked directly to the contact form.
