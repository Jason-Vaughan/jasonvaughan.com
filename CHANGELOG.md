...
- **Implemented card background watermark for RentalClaw** — set `watermarkBackground: true` on the RentalClaw entry in `src/components/OpenClawFleet.jsx`. This displays its red crab claws logo as a giant, dimmed, background watermark (using the same 10% opacity and 1000px size) and hides the smaller icon from the header.
