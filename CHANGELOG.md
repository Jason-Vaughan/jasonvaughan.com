...
- **Enhanced watermark smoke test for RentalClaw proofing** — updated `public/smoke-test.html` to support toggling between Kobold and RentalClaw. The tool updates all layout copy, tags, watermark assets, and color variables dynamically. Replaced the RentalClaw logo with a newly provided transparent version (`rentalclaw_logo.png`) in both the source assets and the static playground assets, resolving background color bleed issues.
