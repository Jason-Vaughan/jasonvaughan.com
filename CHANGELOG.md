...
### Added
- **TiLTClaw built-in-support mention inside the TiLT card** — single-row callout below the TiLT description noting that TiLTClaw (the OpenClaw AI agent triaging TiLT support tickets via Discord) is in production. Small 36px logo, gold left-accent matching TiLT's branding, one-sentence blurb — deliberately compact so it reads as TiLT operational depth, not a competing product. Anchor id (`#tiltclaw`) and share-cards registry entry preserved for deep-linking and the auto-generated OG preview (`/share/tiltclaw/`).
