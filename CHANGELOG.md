...
- **Implemented dialed-in watermark background for Volta** — imported `voltaLogo` and configured `watermarkBackground: true`, `watermarkSize: 1000`, and `watermarkOpacity: 0.1` on the Volta card in `src/components/OpenClawFleet.jsx` based on playground proofing.
