...
- **Added Kobold agent card to OpenClaw Fleet** — integrated the new `Kobold` Tailscale-orchestrated Linux edge node into the `OpenClaw Fleet` list in `src/components/OpenClawFleet.jsx` and added detailed documentation to the RAG database in `public/chat-context.json` so the interview chatbot can address questions about its Lenovo ThinkCentre hardware specs, SSH provisioning workflow, Dockerized gateway, and Copilot (Claude Haiku) backend routing.
