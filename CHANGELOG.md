...
- **Added and refined Kobold agent card** — integrated the new `Kobold` edge node into the `OpenClaw Fleet` list in `src/components/OpenClawFleet.jsx` and the RAG chat database in `public/chat-context.json`. Configured its role as an `AI Assistant Video Engineer` that connects directly to local show networks on-site to scan, monitor, and query local video devices and resources, utilizing Monad-1 over Tailscale as its local LLM backend. Incorporated high-fidelity logo and banner graphic assets and updated its styling with the official Toxic Green accent theme (`#8BC34A`) and copy from the Kobold branding kit. Set the card as a redacted stealth node (`redacted: true`, `REDACTED · Stealth` badge) with abstract hardware specifications to protect project privacy.
