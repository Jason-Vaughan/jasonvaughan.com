...
### Added
- **Interactive ClawHub Stats Dashboard with SVG Sparklines** — refactored `src/components/BuilderStats.jsx` to feature a beautiful, high-impact stats dashboard modeled after clawhub.ai. Fetches live total downloads, skills downloads, and plugins downloads from `clawhub-versions.json` in parallel with git repository stats. Generates mathematically accurate 30-day growth timeseries and renders smooth bezier-curved SVG sparklines with opacity gradient area fills. Implemented a tab switcher (Registry Stats vs Codebase Stats) and a responsive mobile grid design.
