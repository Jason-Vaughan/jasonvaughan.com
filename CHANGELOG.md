...
### Added
- **[TASK-RESUME-2] Stripe AV Events Manager Profile & Tip Jar Logic** — added `stripe` and `stripe2026` to `PASSCODE_CONFIGS` in `src/App.jsx` for targeted recruiter delivery. Automatically unlocks the PDF, sets the EventPro persona, applies the "AV Events Manager" role filter, and displays a custom Stripe welcome banner. Additionally, refactored Tip Jar visibility to conditionally hide the top and bottom support links whenever any custom passcode or recruiter banner is active, keeping the professional landing experience clean.
- **[TASK-RESUME-2] Persona & Passcode-Based Dynamic Landing Customization** — implemented `PASSCODE_CONFIGS` in `src/App.jsx` mapping passcodes (`anthropic`, `tpm2026`, `google`, `eventpro`) directly to target visitor personas (`EventPro`, `Recruiter`), sub-role filters, and custom top sticky welcome banners (e.g. `✨ Welcome Anthropic Hiring Team · AV Production & Broadcast View Unlocked` and `✨ Welcome NVIDIA / Hiring Team · Technical Program Manager View Unlocked`). When a recruiter opens a link with `?pass=tpm2026` or `?pass=anthropic`, the site automatically unlocks the resume gate, selects their persona, applies the sub-role filter, and renders a custom company welcome banner without requiring interactive modal prompts.
- **[TASK-RESUME-1] Pre-Assigned Passwords & URL Query Parameter Auto-Unlock** — implemented pre-assigned password mapping (`RESUME_PASSWORDS`) supporting role/company passcodes (`anthropic`, `anthropic2026`, `google`, `google2026`, `eventpro`, `tpm2026`, `moscone`, `jason2026`). Added automatic URL query parameter auto-unlock (`?pass=anthropic` or `?password=google` or `?code=anthropic`) on mount in `src/App.jsx`. Unlocks the resume gate seamlessly without requiring password input when shared via link, persisting `resumeUnlocked` and `resumePassVariant` to `localStorage`. Added unit tests in `src/resume-gate.test.js` (8 tests passing).
...
- **Narrative-driven About section** — created a centralized narrative store (`src/data/about.js`) and `<About />` component (`src/components/About.jsx`) rendered inside a Collapsible panel when in Preview Mode. Includes a non-chronological story, a 4-pillar grid, philosophy Q&As, a milestone timeline, and an interactive "AI Interview" CTA that opens the chatbot widget. Adds a prominent "Download PDF Resume" button inside the Hero block when in Recruiter Mode, served directly from the root asset index.
...
### Changed
- **Certifications Issuer Logos** — Added issuer logos to the Certifications cards (Issue #82) to give the text-heavy cards visual texture and instant credibility. Gated behind an optional `logo` field with a graceful fallback to text-only when no logo is set. Included clean placeholder SVGs for missing logos (OSHA, IATSE, Barco, etc) that can be overridden later.
- **Skills Section UI Refactored** — Replaced the flat grid of skills with a vertical list of collapsible accordion cards (one per domain group). Addresses issue #81 to keep the section compact as more domains are added.
...
- **Refined virtual interview startup flow** — updated the chat widget initialization in `src/components/ChatWidget.jsx` to intercept the startup trigger client-side, showing a high-impact, static opening message (under 100 words, custom-crafted from first principles) instantly with zero loading delay. It hides the trigger query from the visible chat log while preserving it as a hidden first message in the stack so the LLM retains correct conversational context. Also adds a customized list of 5 evaluation navigation choices when in virtual interview mode.
- **Improved ChatWidget desktop dimensions and scrolling** — widened the chat drawer to 500px and increased its height to 680px on desktop screens (>=640px) in `src/components/ChatWidget.jsx` to improve readability and give suggestions more horizontal space. Also resolved an issue where the initial message auto-scrolled to the bottom, cutting off the top of the virtual interview introduction.
- **Corrected and updated Google ETT role details** — integrated your accurate, detailed first-person description of ETT and Spotlight surrogate TPM duties. Re-aligned `public/chat-context.json`, `src/data/career.js`, and `src/data/portfolioItems.js` to highlight your role bridging product and event production teams, physically managing/operating demo desk devices at Hardware Launches/Google I/O, serving as surrogate TPM for Cloud Next Spotlight keynotes (up to 1,500 people), and coordinating with executive dev teams. Corrected inaccurate references to direct budget forecasting/ownership or vendor selection.
- **Improved ChatWidget API error reporting** — updated the error handling in `src/components/ChatWidget.jsx` to parse and display custom error messages directly from the Cloudflare Worker (such as IP rate limits or max conversation length), rather than swallowing them behind a generic connection error card.
- **Fixed hardcoded Recruiter View banner label and reframed copy** — corrected a bug in `src/App.jsx` where the top sticky banner prefix was hardcoded to `Recruiter View:` regardless of the active persona. It now dynamically displays the active persona's name (e.g. `OpenClaw Mode:`). Also updated the focus descriptions in `src/components/PersonaSelector.jsx` to focus on Jason's individual work, tools, and custom agent fleet rather than sounding like third-party product advertisements.
- **Implemented dynamic persona-specific taglines and AI-Native header badges** — configured the main landing page header to render bio/taglines customized to each active visitor persona. Added a premium, gradient-branded "AI-NATIVE" highlight badge that calls out Jason's deep AI engineering passion, aligning his introduction with the active persona. Set alignment to CENTER, bio paragraph max-width to 900px, and dialed in horizontal badge dimensions with safe mobile wrapping. Hardened header child layout by setting display flex-column with horizontal centering to override text alignment bias. Fixed card layout spacing to add a 24px margin gap between the header elements and adjacent layout cards.
- **Implemented dynamic section sorting by persona priority** — refactored the layout sequence in `src/App.jsx` to dynamically sort and render collapsible sections. Highlighted, persona-relevant cards (like OpenClaw Fleet and ClawHub when in OpenClaw mode, or TiLT and Cierre Sensei in Investor mode) are automatically lifted to the top of the viewport above general history, aligning the page layout directly with the active mode's banner focus. Keeps sub-role filter triggers nested inline correctly.
- **Always render BuilderStats at the top of the page** — updated `src/App.jsx` to render the horizontal developer/builder stats panel at the very top of the content area for all visitor modes (including Recruiter mode), followed immediately by the dynamically prioritized highlighted sections.
- **Added and refined Kobold agent card** — integrated the new `Kobold` edge node into the `OpenClaw Fleet` list in `src/components/OpenClawFleet.jsx` and the RAG chat database in `public/chat-context.json`. Configured its role as a voice-operated, hands-free `AI Assistant Video Engineer` that connects directly to local show networks on-site to scan, monitor, and query local video devices and resources, utilizing Monad-1 over Tailscale as its local LLM backend. Incorporated the official Toxic Green accent theme (`#8BC34A`) and copy from the Kobold branding kit. Formatted its circular reptilian eye avatar as an upscaled (max 1000px) and dimmed card background watermark (10% opacity) rather than a header logo, clipping cleanly at the card's outer borders. Set the card as a redacted stealth node (`redacted: true`, `REDACTED · Stealth` badge) with no banner. Rendered its widescreen advertisement graphic as a clickable thumbnail aligned on the right side of the card (wrapping on mobile), opening a premium blurred zoom modal with an "Apply for Guild Membership" CTA linked directly to the contact form.
- **Enhanced watermark smoke test for agent proofing** — updated `public/smoke-test.html` to support toggling between Kobold, RentalClaw, Volta, and TiLTClaw. The tool updates all layout copy, tags, watermark assets, and color variables dynamically. Copied transparent Volta and TiLTClaw logo assets to playground static folders. Converted and deployed the new transparent V icon for Volta (`20260714-192822-v-for-volta`) as the active logo asset, and updated TiLTClaw to use its official high-res banner logo (`TiLTClaw-Banner-bigger.png`). Added dynamic banner strip rendering at the top of the card replica in the playground for accurate proofing.
- **Created header alignment and positioning playground** — implemented `public/header-smoke.html` to allow dynamic staging and alignment tests for the main tagline text, bio widths, margins, and vertical/horizontal layout modes of the AI-Native badge.
- **Implemented dialed-in watermark background for RentalClaw** — set `watermarkBackground: true`, `watermarkSize: 550`, and `watermarkOpacity: 0.165` on the RentalClaw card in `src/components/OpenClawFleet.jsx` based on playground proofing results. Hides the smaller logo from the header.
- **Implemented dialed-in watermark background for Volta** — imported `voltaLogo` and configured `watermarkBackground: true`, `watermarkSize: 650`, and `watermarkOpacity: 0.1` on the Volta card in `src/components/OpenClawFleet.jsx` based on playground proofing.
...
### Changed
- **TangleClaw "zero-dependency" claim sharpened to "zero npm dependencies"** — the description and the tech-stack tag now say exactly what's true (the repo has no `package.json` at all; runtime is Node built-ins only) without implying the system tools it drives (tmux, ttyd, git, optional Caddy ingress) don't exist — those stay disclosed in the tech-stack tags.
- **TangleBrain hero card surfaces the new PyPI release** — TangleBrain is now published on PyPI (`tanglebrain`, v0.17.0 at time of writing), so the card adds an "On PyPI" chip in the title row and a monospace `pip install tanglebrain` CTA button, both linking to the package page. The version chip keeps auto-fetching from GitHub Releases (no hardcoded version — guarded by the new `src/featured-cards.test.js`). Fallback stat values refreshed to current collector numbers (9K+ LoC / 400+ tests / 60 commits).
- **TangleClaw hero card copy updated for the 4.0 release** — the description was V3-era; it now leads with the 4.x surface (session continuity with wrap protocols, Project Master control plane, orchestration profiles, secured remote access) on top of the existing dashboard/ttyd/methodology/shared-docs/port-management feature list. The version chip already tracks GitHub Releases so it shows v4.0.0 automatically. Stale pre-fetch fallback stats bumped (39K+ → 100K+ LoC, 1,520 → 3,900+ tests, 150+ → 370+ commits). Screenshots remain V3-era until new 4.0 captures land in project-assets.
...
- **Section-level (category) copy-link buttons** — each dropdown header has a compact copy-link button that copies a clean `/#<section>` deep link, so a whole category (Projects, Skills, …) can be shared and opens + flashes on arrival. Card-level links keep their OG-rich `/share/<id>/` form.
- **New Certifications section — curated "experience-credentialed" proof set** — added `src/components/Certifications.jsx` driven by `src/data/certifications.js`, in the same dark-card style, placed right after Skills. Deliberately short and current (stale 15-20 yr Microsoft/Cisco/Citrix certs omitted — they age rather than credit). Subtext *"More than 25 years of field-verified credentials — and still leveling up."* conveys depth + currency with no anchor year (avoids age-math) and never references the absence of a degree. Cards: 7-year IATSE Local 16 stagecraft apprenticeship (rigging, carpentry, audio, video, show management), Barco Video Engineering Level A, OSHA 30, Google Project Management Professional Certificate (Coursera, 2026 — the only dated cert, signaling currency), and an IATSE Local 16 Training Trust Fund instructor credential. IATSE items framed factually as training/credentials, never as union advocacy. Optional `year` chip and `detail` sub-line render only when set. Data contract guarded by `src/data/certifications.test.js` (5 tests).
...
### Fixed
- **Section-level deep links now visibly flash** — the amber highlight pulse is a box-shadow ring that's visible on a card but invisible on a large, transparent `<section>`, so opening a whole category (`/#projects`, `/#skills`, …) scrolled but didn't appear to flash. Section-level links now pulse the dropdown **header panel** (a defined, visible element) while card-level links keep flashing the card itself. Browser-verified.
- **Copy-link now works on insecure origins (LAN / tailnet HTTP)** — `ShareLink` previously called only `navigator.clipboard.writeText`, which browsers block outside a secure context (non-HTTPS, non-`localhost`), so the button silently did nothing when the site was viewed over a plain-HTTP tailnet IP. Added a secure-context guard plus a legacy `execCommand("copy")` textarea fallback, so copy succeeds (and shows "Copied") everywhere. The live HTTPS site was never affected.
...
### Changed
- **Medusa card now reads as a general coordination fabric** — added a line positioning Medusa beyond IDE agents: parallel AI sessions coordinating (a direction for the next TangleClaw), fleet-wide task fan-out, and cross-agent verification via consensus. Each example is grounded in a real Medusa capability (PortHub discovery, A2A swarm, gossip consensus) rather than aspirational fluff.
- **Medusa card cross-references TangleClaw** — added a feature line noting Medusa's automatic peer discovery via TangleClaw PortHub (per Medusa's README), surfacing the self-hosted AI-ops ecosystem fit. (UCI intentionally not cross-referenced — no integration exists in Medusa's README.)
- **Reframed the Lines Refactored tooltip** — anchors the number to Lines Authored (lifetime) rather than current Lines of Code, so visitors don't misread "refactored ÷ alive code" as most of the codebase being thrown away. Frames it as healthy iteration ("revisited, not just stacked on").
- **Sharpened the TangleBrain hero card copy** — refactored the blurb to match the upgraded TangleBrain README: leads with the "cloud-by-default routing" problem, and surfaces the two real differentiators that were understated — the OAuth/local-first credentials posture (raw API keys gated, never injected into a CLI) and prompt-aware routing (classifier sends grunt work to the free local model). Also notes it runs standalone or drops in alongside TangleClaw.
...
- **ClawHub cards: clearer download pill + hover stats panel** — the download chip now reads "245 downloads" (was a cryptic "↓ 245"), and hovering any card reveals a popover with version, downloads, stars, security-scan status (Passed/Pending), and last-published date. The extra fields (`stars`, `security`, `lastPublished`) are captured live by the watcher (project-assets#24). Card `overflow:hidden` was dropped so the popover can float (accent bar rounds its own corners, same as BuilderStats).
